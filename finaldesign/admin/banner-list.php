<?php
    ...
    $sql_select = "SELECT id,banner,banner_sort,banner_link,banner_desc,banner_state FROM `wyp_banner` WHERE 1";
...
<html>
    ...
    <body>
        ...
        <div class="x-body">
            <xblock><button class="layui-btn layui-btn-danger" onclick="delAll()"><i class="layui-icon">&#xe640;</i>批量删除</button><button class="layui-btn" onclick="banner_add('添加用户','banner-add.html','600','500')"><i class="layui-icon">&#xe608;</i>添加</button><span class="x-right" style="line-height:40px">共有数据：<?=$num?> 条</span></xblock>
            <table class="layui-table">
                ...
                <tbody id="x-img">
                    ...
                    <tr>
                        ...
                        <td><?=$v['id']?></td>
                        <td><img  src="<?=$v['banner']?>" width="200" alt="" height="50"></td>
                        <td><?=$v['banner_sort']?></td>
                        <td><?=$v['banner_link']?></td>
                        <td><?=$v['banner_desc']?></td>
                        <td class="td-status">
                        ...
                        <td class="td-manage">
                            ...
                            <a title="编辑" href="javascript:;" onclick="banner_edit('编辑','banner-edit.php?id=<?=$v['id']?>','<?=$v['id']?>','','510')"
        ...
        <script>
            ...
            //批量删除提交
             function delAll () {
                layer.confirm('确认要删除吗？',function(index){
                    ...
                    str = arr.join(',');
                    $.ajax({
                        url:'../common/ajax.php?type=delAllbanner',
                        data:{ids:str},
                        type:'post',
                        dataType:'json',
            ...
            function banner(obj,id,state){
                $.ajax({
                    url:'../common/ajax.php?type=update_banner_state',
                    data:{
                        id:id,
                        banner_state:state,
                    },
            ...
            function banner_del(obj,id){
                layer.confirm('确认要删除吗？',function(index){
                     $.ajax({
                        url:'../common/ajax.php?type=del_banner',
                        data:{id:id},
                        type:'post',
                        dataType:'json',
                        success:function(data){
